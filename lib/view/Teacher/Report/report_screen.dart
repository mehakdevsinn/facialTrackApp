        ...
        children: [
          ...
          Row(
            children: [
              ...
              SizedBox(width: 5),
              Expanded(child: Text(value, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
...
void _generateAndDownloadReport(BuildContext context) async {
  ...
  try {
    ...
    String csvData = "Name,Present,Absent,Percentage\nJane Doe,22,2,91%\nLisa Davis,18,6,75%\nMike King,15,9,62%";
          ...
          Row(
            ...
            children: [
              CircularPercentIndicator(
                ...
                lineWidth: 10.0,
                percent: 0.82,
                center: Text("82%", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24)),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              const Text("Class Performance", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
              Text("Sorted by %", style: TextStyle(color: primaryDark, fontSize: 12, fontWeight: FontWeight.w500)),
            ],
          ),
          // In _buildStudentListCard inside Column children:
const SizedBox(height: 15),
if (!showOnlyLow) _studentRowUnique("Jane Doe", 90), // Show only if filter is OFF
...
},              child: Text("View Complete Report", style: TextStyle(color: primaryDark, fontWeight: FontWeight.bold)),
            ),
          )
        ],
      ),
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text(name[0], style: TextStyle(color: primaryDark, fontWeight: FontWeight.bold)),
          ),
          ...
          Container(
            ...
            child: Center(
              child: Text(isLow ? "LOW" : "OKAY", style: TextStyle(color: statusColor, fontSize: 10, fontWeight: FontWeight.bold)),
            ),
          )
        ],
      ),
...
Widget _buildTrendCard() {
  const double maxChartHeight = 150.0; // Height thodi badha di hai taaki overflow na ho
            ...
            Container(
              ...
              child: Column(
                ...
                children: ["100%", "80%", "60%", "40%", "20%", "0%"]
                    .map((label) => Text(label, style: TextStyle(fontSize: 9, color: Colors.grey)))
                    .toList(),
              ),
            ),
            SizedBox(width: 10),
...
_buildScaledBar(85, 0, 0, "Week 1", maxChartHeight, isHighlighted: !showOnlyLow),
_buildScaledBar(10, 20, 45, "Week 2", maxChartHeight, isHighlighted: true), // Always show low
_buildScaledBar(20, 15, 30, "Week 3", maxChartHeight, isHighlighted: !showOnlyLow),
_buildScaledBar(0, 35, 50, "Week 4", maxChartHeight, isHighlighted: !showOnlyLow),
_buildScaledBar(0, 10, 15, "Week 5", maxChartHeight, isHighlighted: true), // Always show low
  ],
                    ),
                  ),
                ],
...
Widget _buildScaledBar(double bluePct, double orangePct, double tealPct, String label, double maxHeight, {bool isHighlighted = true}) {
  double bH = (bluePct / 100) * maxHeight;
  double oH = (orangePct / 100) * maxHeight;
  double tH = (tealPct / 100) * maxHeight;

  return Opacity(
    opacity: isHighlighted ? 1.0 : 0.2, // Low click karne par baki bars fade ho jayengi
    child: Column(
      ...
      children: [
        Container(
          ...
          child: Column(
            children: [
              if (tealPct > 0) Container(height: tH, decoration: BoxDecoration(color: Color(0xFF26A69A), borderRadius: BorderRadius.vertical(top: Radius.circular(2)))),
              if (orangePct > 0) Container(height: oH, color: Color(0xFFFF8A65)),
              if (bluePct > 0) Container(height: bH, color: Color(0xFF1A4B8F)),
            ],
          ),
        ),
        SizedBox(height: 8),
